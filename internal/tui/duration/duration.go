...
type Model struct {
	...
	width    int
	height   int
	selected int // -1 means back, otherwise the duration
}
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "enter", " ":
			m.selected = options[m.cursor].duration
			return m, tea.Quit
		}
	}
...
func (m Model) View() string {
	...
	s := components.HeaderWithWidth("TIMED TEST", "Choose your test duration", m.styles, m.width)
	s += "\n\n"
